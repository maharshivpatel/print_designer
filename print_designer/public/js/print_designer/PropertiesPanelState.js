	...
	MainStore.propertiesPanel.push({
		...
		sectionCondtional: () => {
			if (
				...
				ElementStore.isElementOverlapping(MainStore.getCurrentElementsValues[0])
			) {
				return false;
			}
			if (
				(MainStore.isRawPrintEnable && MainStore.getCurrentElementsId.length == 1) ||
		...
		fields: [
			{
				...
				frappeControl: (ref, name) => {
					...
					makeFeild({
						...
						options: () => [
							{ label: "Yes", value: "Yes", is_selected: true },
							{ label: "No", value: "No" },
						],
						...
						onChangeCallback: (value = null) => {
							if (value) {
								MainStore.isRawPrintEnable = value === "Yes";
			...
			},
			{
				...
				isLabelled: true,
				condtional: () =>
					MainStore.getCurrentElementsId.length < 1 && MainStore.isRawPrintEnable,
				frappeControl: (ref, name) => {
					const MainStore = useMainStore();
					makeFeild({
						name: name,
